...
class ESSEScatterModel:
    ...
    def get_depth_map(self,attenuation_map: torch.Tensor) -> torch.Tensor:
        dx = self.object_meta.dx
        print(f"Original dx: {dx}")

        if self.CGSM and self.CGSM > 1:
            dx *= self.CGSM

        # cumulative attenuation from detector side
        optical_depth = (torch.cumsum(attenuation_map, dim=0)) * dx

        tau = optical_depth / self.mu_water
        return tau
